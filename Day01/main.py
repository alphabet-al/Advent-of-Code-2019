...
requires zero fuel; the remaining mass, if any, is instead handled by wishing really hard, which has no mass and is outside the scope 
of this calculation.
...
A module of mass 14 requires 2 fuel. This fuel requires no further fuel (2 divided by 3 and rounded down is 0, which would call for 
...
which requires 21 fuel, which requires 5 fuel, which requires no further fuel. So, the total fuel required for a module of mass 1969 is 
654 + 216 + 70 + 21 + 5 = 966.
...
fuel? (Calculate the fuel requirements for each module separately, then add them all up at the end.)
'''
import math
...
class Module:
    def __init__(self, mass):
        self.mass = int(mass)
        self.fuel = Module.fuel_need(self.mass)

    @staticmethod
    def fuel_need(inmass):
        if inmass <= 0:
            return 0
        else:
            calc = math.floor(inmass / 3) - 2
            if calc >= 0:
                return (calc + Module.fuel_need(calc))
            else:
                return 0
